...
randomize.addEventListener('click', result);    // 为random变量添加监听器

function result() {
    let newStory = storyText;
    let xItem = randomValueFromArray(insertX);  //生成随机文字变量
    let yItem = randomValueFromArray(insertY);
    let zItem = randomValueFromArray(insertZ);

    //替换newStory里文字
    newStory = newStory.replace(':inserta:', xItem);    //哪怕是同一个文字段也需要替换两次
